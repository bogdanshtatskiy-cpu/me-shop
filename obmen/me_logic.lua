...
local me = {}
local chests = {}
local input_chest = nil -- Левый сундук (запомнится навсегда)
...
function me.init()
    ...
    -- Ищем правый сундук (через контроллер инвентаря)
    for s = 0, 5 do
    ...
    -- Пытаемся вспомнить адрес левого сундука (если уже сканировали ранее)
    local f = io.open("/home/left_chest.cfg", "r")
    if f then
        local saved_addr = f:read("*a")
        f:close()
        for _, c in ipairs(chests) do
            if c.address == saved_addr then
                input_chest = c
                break
            end
        end
...
function me.getScanItems()
    -- 1. Сначала точно находим СЛИТОК в правом сундуке (через Контроллер Инвентаря)
    local out_item = nil
    local out_label = nil
    ...
    if not out_item then return nil, nil end -- В правом пусто

    -- 2. Если левый сундук еще не привязан - ищем его и ЗАПОМИНАЕМ!
    if not input_chest then
        for _, chest in ipairs(chests) do
            ...
            if ok and type(stacks) == "table" then
                for _, data in pairs(stacks) do
                    if type(data) == "table" and data.id then
                        ...
                        -- Сравниваем именно ИМЕНА ("Железный слиток" == "Железный слиток"), чтобы избежать ошибок ID
            ...
            -- Если в сундуке есть предметы, но СЛИТКА там нет -> Значит это Левый сундук с рудой!
            if has_any_item and not has_output_item then
                input_chest = chest
                -- Записываем адрес левого сундука на жесткий диск навсегда
                local f = io.open("/home/left_chest.cfg", "w")
                if f then
                    f:write(chest.address)
                    f:close()
                end
                break
            end
    ...
    -- 3. Читаем РУДУ из найденного Левого сундука
    local in_item = nil
    if input_chest then
...
function me.processOneExchange(trades)
    if not input_chest then return false end -- Если сундук еще не привязан сканированием
    ...
    for slot, data in pairs(stacks) do
        if type(data) == "table" and data.id then
            for _, t in ipairs(trades) do
                -- Сравниваем OP формат с сохраненным
                if data.id == t.input.name and math.floor(data.dmg or 0) == math.floor(t.input.damage or 0) then
                    ...
                    if can_process > 0 then
                        ...
                        if pushed_ore and pushed_ore > 0 then
                            ...
                            return true, false, "МЭ не принимает руду", 0, t, can_process
                        end
                    end
                end
            end
